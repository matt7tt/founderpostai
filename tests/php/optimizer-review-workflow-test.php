...
FounderPostAI_AISuite_SEO_Store::$rows[101] = (object) array(
	'id'              => 101,
	'post_id'         => 1,
	'field'           => 'title',
...
$row     = FounderPostAI_AISuite_SEO_Store::$rows[101];
...
if ( is_wp_error( $undone ) || 'Original title' !== $seo_values[1]['title'] || 'undone' !== FounderPostAI_AISuite_SEO_Store::$rows[101]->status ) {
	fwrite( STDERR, "FAIL: metadata undo did not restore the previous value\n" );
	exit( 1 );
}

FounderPostAI_AISuite_SEO_Store::$rows[102] = (object) array(
	'id'              => 102,
	'post_id'         => 1,
	'field'           => 'description',
...
if ( ! is_wp_error( $blocked ) || 'founderpostai_aisuite_seo_undo_stale' !== $blocked->get_error_code() || 'approved' !== FounderPostAI_AISuite_SEO_Store::$rows[102]->status ) {
	fwrite( STDERR, "FAIL: undo overwrote or accepted a newer metadata edit\n" );
	exit( 1 );
}

FounderPostAI_AISuite_SEO_Store::$rows[103] = (object) array(
	'id'              => 103,
	'post_id'         => 1,
...
if ( is_wp_error( $link_applied ) || false === strpos( $posts[1]->post_content, '<a href=' ) || $original_content !== FounderPostAI_AISuite_SEO_Store::$rows[103]->rollback_value ) {
	$detail = is_wp_error( $link_applied ) ? $link_applied->get_error_code() : json_encode( array( $posts[1]->post_content, FounderPostAI_AISuite_SEO_Store::$rows[103] ) );
