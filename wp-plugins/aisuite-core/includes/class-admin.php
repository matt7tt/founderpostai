...
class AISuite_Admin {
	...
	public function connection_notice() {
		...
		printf(
			...
			esc_html__( 'AI Suite is installed but not connected yet.', 'aisuite-core' ),
			esc_url( admin_url( 'admin.php?page=' . self::SLUG ) ),
			esc_html__( 'Connect your account', 'aisuite-core' )
		);
	}

	public function render() {
		if ( ! current_user_can( self::CAP ) ) {
			wp_die( esc_html__( 'You do not have permission to manage AI Suite.', 'aisuite-core' ) );
		}
		...
		<div class="wrap aisuite-wrap">
			<h1><?php esc_html_e( 'AI Suite', 'aisuite-core' ); ?></h1>

			<?php $this->render_flash(); ?>

			<div class="aisuite-panel">
				<h2><?php esc_html_e( 'Connection', 'aisuite-core' ); ?></h2>
				...
				<?php if ( $connected ) : ?>
					<p class="aisuite-status aisuite-status--ok">
						...
						printf(
							...
							esc_html__( 'Connected as %s', 'aisuite-core' ),
					...
					<div class="aisuite-actions">
						<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
							...
							<?php submit_button( __( 'Test connection', 'aisuite-core' ), 'secondary', 'submit', false ); ?>
						</form>

						<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
							<?php wp_nonce_field( 'aisuite_disconnect' ); ?>
							<input type="hidden" name="action" value="aisuite_disconnect" />
							<?php submit_button( __( 'Disconnect this site', 'aisuite-core' ), 'secondary', 'submit', false ); ?>
						</form>
					</div>
				<?php else : ?>
					<p>
						<?php esc_html_e( 'Paste the connection code from your AI Suite dashboard. It is single-use and expires in 15 minutes.', 'aisuite-core' ); ?>
					</p>
					...
					<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
						...
						<p>
							<label class="screen-reader-text" for="aisuite-token"><?php esc_html_e( 'Connection code', 'aisuite-core' ); ?></label>
							<input type="text" id="aisuite-token" name="connect_token" class="regular-text" autocomplete="off" required />
						</p>
						<?php submit_button( __( 'Connect site', 'aisuite-core' ) ); ?>
					</form>
				<?php endif; ?>
				...
				<p class="description">
					...
					printf(
						...
						esc_html__( 'Content you submit for processing is sent to %s. Nothing is sent until you run an action.', 'aisuite-core' ),
			...
			<div class="aisuite-panel">
				<h2><?php esc_html_e( 'Brand context', 'aisuite-core' ); ?></h2>
				<p class="description">
					<?php esc_html_e( 'Every module reads this. Fill it in once and results stop sounding generic.', 'aisuite-core' ); ?>
				</p>
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<table class="form-table" role="presentation">
						<tr>
							<th scope="row"><label for="aisuite-what"><?php esc_html_e( 'What the business does', 'aisuite-core' ); ?></label></th>
							<td><textarea id="aisuite-what" name="what_you_do" rows="3" class="large-text"><?php echo esc_textarea( $brand['what_you_do'] ); ?></textarea></td>
						</tr>
						<tr>
							<th scope="row"><label for="aisuite-audience"><?php esc_html_e( 'Who it sells to', 'aisuite-core' ); ?></label></th>
							<td><textarea id="aisuite-audience" name="audience" rows="3" class="large-text"><?php echo esc_textarea( $brand['audience'] ); ?></textarea></td>
						</tr>
						<tr>
							<th scope="row"><label for="aisuite-tone"><?php esc_html_e( 'Tone', 'aisuite-core' ); ?></label></th>
							<td><input type="text" id="aisuite-tone" name="tone" class="regular-text" value="<?php echo esc_attr( $brand['tone'] ); ?>" /></td>
						</tr>
						<tr>
							<th scope="row"><label for="aisuite-market"><?php esc_html_e( 'Primary market', 'aisuite-core' ); ?></label></th>
						...
						<tr>
							<th scope="row"><label for="aisuite-avoid"><?php esc_html_e( 'Words to avoid', 'aisuite-core' ); ?></label></th>
							<td>
								<input type="text" id="aisuite-avoid" name="avoid_phrases" class="large-text" value="<?php echo esc_attr( $brand['avoid_phrases'] ); ?>" />
								<p class="description"><?php esc_html_e( 'Comma separated. Claims you legally cannot make belong here.', 'aisuite-core' ); ?></p>
							</td>
						</tr>
						<tr>
							<th scope="row"><label for="aisuite-style"><?php esc_html_e( 'Style notes', 'aisuite-core' ); ?></label></th>
					...
					<?php submit_button( __( 'Save brand context', 'aisuite-core' ) ); ?>
				</form>
			</div>
	...
	protected function render_usage( array $account ) {
		$plan = isset( $account['plan'] ) ? $account['plan'] : __( 'Free', 'aisuite-core' );
		...
		if ( AISuite_Billing::is_byok() ) {
			...
			<div class="aisuite-meter aisuite-meter--byok">
				<div class="aisuite-meter__head">
					...
					<span class="aisuite-meter__label"><?php esc_html_e( 'actions run this month', 'aisuite-core' ); ?></span>
				</div>
				<p class="aisuite-meter__plan">
					<?php
					printf(
						/* translators: %s: plan name */
						esc_html__( '%s — model usage is billed to your own provider account.', 'aisuite-core' ),
						esc_html( $plan )
					);
					?>
				</p>
		...
		<div class="aisuite-meter">
			<div class="aisuite-meter__head">
				...
				<span class="aisuite-meter__label"><?php esc_html_e( 'actions left this month', 'aisuite-core' ); ?></span>
			</div>
			<div class="aisuite-meter__track" role="img" aria-label="<?php echo esc_attr( sprintf( /* translators: %d: percentage */ __( '%d%% of monthly actions remaining', 'aisuite-core' ), $percent ) ); ?>">
				<div class="aisuite-meter__fill" style="width: <?php echo esc_attr( $percent ); ?>%"></div>
			</div>
			<p class="aisuite-meter__plan"><?php echo esc_html( sprintf( /* translators: %s: plan name */ __( 'Plan: %s', 'aisuite-core' ), $plan ) ); ?></p>
		</div>
		<?php
	}
	...
	protected function render_health() {
		...
		$labels = array(
			'action_scheduler' => __( 'Action Scheduler', 'aisuite-core' ),
			'loopback'         => __( 'Loopback requests', 'aisuite-core' ),
			'wp_cron'          => __( 'WP-Cron', 'aisuite-core' ),
		);
		?>
		<ul class="aisuite-health">
			<li>
				<?php esc_html_e( 'Background jobs run via', 'aisuite-core' ); ?>
				<strong><?php echo esc_html( isset( $labels[ $mode ] ) ? $labels[ $mode ] : $mode ); ?></strong>
			</li>
			<li>
				<?php esc_html_e( 'Results delivered by callback', 'aisuite-core' ); ?>
				<?php if ( ! empty( $health['ok'] ) ) : ?>
					<strong class="aisuite-ok"><?php esc_html_e( 'reachable', 'aisuite-core' ); ?></strong>
				<?php elseif ( isset( $health['ok'] ) ) : ?>
					<strong class="aisuite-warn"><?php esc_html_e( 'blocked — falling back to polling', 'aisuite-core' ); ?></strong>
				<?php else : ?>
					<strong class="aisuite-muted"><?php esc_html_e( 'not tested yet', 'aisuite-core' ); ?></strong>
				<?php endif; ?>
			</li>
		</ul>
		<?php
	...
	protected function render_billing() {
		...
		?>
		<div class="aisuite-panel">
			<h2><?php esc_html_e( 'Billing', 'aisuite-core' ); ?></h2>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<fieldset class="aisuite-modes">
					<legend class="screen-reader-text"><?php esc_html_e( 'Billing mode', 'aisuite-core' ); ?></legend>
					...
					<label class="aisuite-mode">
						...
						<span class="aisuite-mode__title"><?php esc_html_e( 'Included actions', 'aisuite-core' ); ?></span>
						<span class="aisuite-mode__desc"><?php esc_html_e( 'Your plan includes a monthly allowance. Nothing else to set up.', 'aisuite-core' ); ?></span>
					</label>

					<label class="aisuite-mode">
						<input type="radio" name="mode" value="<?php echo esc_attr( AISuite_Billing::MODE_BYOK ); ?>" <?php checked( AISuite_Billing::MODE_BYOK, $mode ); ?> />
						<span class="aisuite-mode__title"><?php esc_html_e( 'Use my own API key', 'aisuite-core' ); ?></span>
						<span class="aisuite-mode__desc"><?php esc_html_e( 'Flat plan fee, unlimited actions, model usage billed directly by your provider.', 'aisuite-core' ); ?></span>
					</label>
				</fieldset>

				<?php submit_button( __( 'Save billing mode', 'aisuite-core' ), 'secondary', 'submit', false ); ?>
			</form>

			<?php if ( AISuite_Billing::MODE_BYOK === $mode ) : ?>
				<hr />
				<h3><?php esc_html_e( 'Provider key', 'aisuite-core' ); ?></h3>
				...
				<?php if ( ! empty( $key['present'] ) ) : ?>
					<p class="aisuite-status aisuite-status--ok">
						...
						printf(
							...
							esc_html__( '%1$s key ending %2$s is active.', 'aisuite-core' ),
					...
					<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
						...
						<?php submit_button( __( 'Remove key', 'aisuite-core' ), 'secondary', 'submit', false ); ?>
					</form>
				<?php else : ?>
					<p>
						<?php
						printf(
							...
							esc_html__( 'The safest place to add your key is your account dashboard: %s. Adding it here works too, but the key passes through this website on its way to us.', 'aisuite-core' ),
							'<a href="' . esc_url( $dash ) . '" target="_blank" rel="noopener">' . esc_html__( 'open dashboard', 'aisuite-core' ) . '</a>'
						);
						?>
					</p>
					...
					<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" autocomplete="off">
						...
						<p>
							<label for="aisuite-provider"><?php esc_html_e( 'Provider', 'aisuite-core' ); ?></label><br />
						...
						<p>
							<label for="aisuite-key"><?php esc_html_e( 'API key', 'aisuite-core' ); ?></label><br />
						...
						<p class="description">
							<?php esc_html_e( 'The key is sent to the gateway and verified there. It is not saved in this WordPress site, so it never appears in your database, backups, or staging copies.', 'aisuite-core' ); ?>
						</p>

						<?php submit_button( __( 'Save key', 'aisuite-core' ), 'primary', 'submit', false ); ?>
					</form>
				<?php endif; ?>
			<?php endif; ?>
		</div>
	...
	protected function render_modules() {
		...
		?>
		<div class="aisuite-panel">
			<h2><?php esc_html_e( 'Modules', 'aisuite-core' ); ?></h2>

			<?php if ( empty( $modules ) ) : ?>
				<p><?php esc_html_e( 'No modules installed yet. Install AI Suite SEO to start.', 'aisuite-core' ); ?></p>
			<?php else : ?>
					...
					<?php foreach ( $modules as $slug => $module ) : ?>
						<li>
							...
							<?php if ( $module['settings_url'] ) : ?>
								<a href="<?php echo esc_url( $module['settings_url'] ); ?>"><?php esc_html_e( 'Open', 'aisuite-core' ); ?></a>
							<?php endif; ?>
						</li>
					<?php endforeach; ?>
				</ul>
			...
			<?php if ( 'wp_cron' === AISuite_Job_Queue::dispatch_mode() ) : ?>
				<p class="description">
					<?php esc_html_e( 'Loopback requests are disabled on this site, so background jobs wait for WP-Cron, which only fires when someone visits. On a quiet site, results can take a few minutes.', 'aisuite-core' ); ?>
				</p>
			<?php endif; ?>
		</div>
		<?php
	...
	protected function render_flash() {
		...
		$success = array(
			'connected'     => __( 'Site connected.', 'aisuite-core' ),
			'disconnected'  => __( 'Site disconnected. Stored credentials were removed.', 'aisuite-core' ),
			'brand_saved'   => __( 'Brand context saved.', 'aisuite-core' ),
			'billing_saved' => __( 'Billing mode updated.', 'aisuite-core' ),
			'key_saved'     => __( 'Key saved and verified. It is stored on the gateway, not in this site.', 'aisuite-core' ),
			'key_removed'   => __( 'Key removed.', 'aisuite-core' ),
			'health_ok'     => __( 'Connection works in both directions.', 'aisuite-core' ),
		);
		...
		if ( isset( $success[ $code ] ) ) {
			...
			$type    = 'warning';
			$message = __( 'We can reach the gateway, but its replies are being blocked before they arrive. Jobs will still finish — results just take a few minutes longer while we poll for them.', 'aisuite-core' );
		} else {
			return;
		}
	...
	protected function guard( $action ) {
		if ( ! current_user_can( self::CAP ) ) {
			wp_die( esc_html__( 'You do not have permission to do that.', 'aisuite-core' ) );
		}
